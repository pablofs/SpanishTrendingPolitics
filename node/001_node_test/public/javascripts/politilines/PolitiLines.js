...
politi.model.DebateData = function(data,dl) {
	...
	if(this.loadable) this._url = "data/politilines/" + data.url + "?" + Math.floor(Math.random() * Math.pow(10,10));
	this._loaded = false;
}
...
politi.view.CandidatePlotTooltip = function(anchr) {
	...
	var showTmr = null;
	content.addClass("plot_tip").html("This sparkline indicates the volume <br>of candidate&apos;s sentences used over <br>the course of the debate. Specific <br>word appearances are highlighted <br>upon interaction with the Top Words.").appendTo(body);
...
politi.model.DebateCategories.prototype.load = function() {
	if(!this._inited) {
		this._inited = true;
		var http = new haxe.Http("data/politilines/debates.json" + "?" + Math.floor(Math.random() * Math.pow(10,10)));
		http.onData = $closure(this,"handleData");
		// http.request(true);
		http.request(false);
	}
...
politi.view.KeyBead = function(p) {
	...
	this.gOpen = politi.view.VizView.vector_layer.image("images/politilines/open" + type,0,0,13,13);
	this.gClose = politi.view.VizView.vector_layer.image("images/politilines/close" + type,0,0,13,13);
...
politi.view.WordNode.tip = null;
politi.model.DebateData.DIRECTORY = "data/politilines/";
...
politi.model.DebateCategories.PATH = "data/politilines/debates.json";
